...
namespace CAF_CUP.Tests
{
    public abstract class CAF_CUPTestBase : AbpIntegratedTestBase<CAF_CUPTestModule>
    {
        ...
        protected void UsingDbContext(Action<CAF_CUPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CAF_CUPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CAF_CUPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CAF_CUPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CAF_CUPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CAF_CUPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CAF_CUPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CAF_CUPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CAF_CUPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CAF_CUPDbContext, Task<T>> func)
        {
            T result;
